...
export class ApiService {
	...
	async getChainDataForSelectedParticipant(): Promise<
		ChainQuestionnaire | undefined
	> {
	...
	async getChainQuestionnaireId(): Promise<number | undefined> {
		...
		const chainQuestionnaireId = await AsyncStorage.getItem(
	...
	): Promise<ChainQuestionnaire | undefined> {
	...
	async addChainData(data: ChainQuestionnaire) {
		...
		if (!(participant || dataHasParticipantId)) {
			console.error(
				"Cannot save the chain data. No participant found to submit the data for."
			);
			return;
		}
		...
		if (!isConnected) {
			...
			await AsyncStorage.setItem(
				"chain_data_draft_" + participantId,
				JSON.stringify(data)
			);
		}
	...
	async selectParticipant(
		...
		if (user) {
			...
			if (user.participants && user.participants.length > 0) {
				...
				const participant = dependents.find(
	...
	async getSelectedParticipant(): Promise<Participant | undefined> {
		...
		if (user) {
			if (user.participants && user.participants.length > 0) {
				...
				const cachedParticipantJson = await AsyncStorage.getItem(
					"selected_participant"
				);
				...
				if (cachedParticipantJson) {
					const cachedParticipant = JSON.parse(
						cachedParticipantJson
					) as Participant;

					if (
						cachedParticipant &&
						cachedParticipant.hasOwnProperty("id")
					) {
						const updatedParticipant = dependents.find(
