...
class MasteryService {
  ...
  static getMasteryInfoForStep(chainData: ChainQuestionnaire, chainStepId: number): MasteryInfo {
    const masteryInfo: MasteryInfo = {
      chainStepId,
      stepStatus: ChainStepStatus.not_complete,
    };

    const steps = this.getAllStepAttemptsForChainStepName(chainData, chainStepId);
    ...
    if (steps.length > 0) {
      ...
      const masteredStepIndex = steps.findIndex(s => s.status === ChainStepStatus.mastered);
      if (masteredStepIndex !== -1) {
        ...
        const sessionTypes = [ChainSessionType.probe, ChainSessionType.training];

        let boosterStepIndex = -1;
        sessionTypes.forEach(sessionType => {
          if (!masteryInfo.dateBoosterInitiated) {
            boosterStepIndex = this.findIndexBoosterIntroduced(steps.slice(masteredStepIndex), sessionType);
            if (boosterStepIndex !== -1) {
              masteryInfo.stepStatus = ChainStepStatus.focus;
              masteryInfo.dateBoosterInitiated = stepsAfterMastered[boosterStepIndex].date;
            }
          }
        });
  ...
  static _sortSessionsByDate(unsortedSessions: ChainSession[]): ChainSession[] {
   ...
   */
  static findIndexBoosterIntroduced(stepAttempts: StepAttempt[], sessionType: ChainSessionType): number {
  ...
  static shouldFocusOnStep(chainQuestionnaire: ChainQuestionnaire, chainStepId: number, numAttempts: number): boolean {
    ...
    this._sortSessionsByDate(chainQuestionnaire.sessions).forEach(session => {
      session.step_attempts.forEach(stepAttempt => {
